...
function renderPlayers(){ $('playerCount').textContent=`${players.length}명`; $('playerList').innerHTML=players.length?players.map(p=>`<div class="player"><div class="player-main"><div class="player-name">${esc(p.name)}</div><div class="meta"><span class="pill">${p.gender}</span><span class="pill"></span>${p.fixedPartner?`<span class="pill">파트너 ${esc(p.fixedPartner)}</span>`:''}</div></div><div class="row"><select style="width:70px" onchange="changePlayer('${esc(p.name)}','level',this.value)">${[1,2,3,4,5].map(n=>`<option ${n===+p.level?'selected':''}>${n}</option>`).join('')}</select><button class="btn danger small" onclick="removePlayer('${esc(p.name)}')">삭제</button></div></div>`).join(''):'<div class="empty">참가자를 추가하거나 저장 명단에서 선택하세요.</div>'}
...
window.openShareView=()=>{if(!schedule.length)return toast('먼저 대진을 생성하세요.');const courts=+$('courtCount').value;const now=new Date();const date=`${now.getFullYear()}.${String(now.getMonth()+1).padStart(2,'0')}.${String(now.getDate()).padStart(2,'0')}`;let html=`<h1 class="share-title">🎾 테니스 복식 대진표</h1><div class="share-sub">${date} · 총 ${schedule.length}경기 · ${players.length}명 참가</div><div class="share-grid" style="--share-cols:${courts===1?1:2}">`;for(let c=1;c<=courts;c++){const ms=schedule.filter(m=>m.court===c);html+=`<section class="share-court"><h3>${c}코트 · ${[...courtRules[c]].join('+')}</h3>${ms.length?ms.map(shareMatchHTML).join(''):'<div class="share-match"><div class="share-team">배정 경기 없음</div></div>'}</section>`}html+='</div><div class="share-hint">스크린샷 준비를 누르면 조작 버튼과 이 안내문이 숨겨집니다.</div>';$('shareWrap').innerHTML=html;const ov=$('shareOverlay');ov.classList.add('show');ov.classList.remove('clean','dense');if(schedule.length>=7)ov.classList.add('dense');document.body.style.overflow='hidden'};
